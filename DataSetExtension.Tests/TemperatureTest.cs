...
namespace DataSetExtension.Tests
{
	...
	public class TemperatureTest
	{
		...
		public void ToStringTest()
		{
			...
			record = new Temperature { Value = 81, Date = DateTime.Parse("2/14/1997") };
			Assert.That(record.ToString(0), Is.EqualTo("970214 00 A  272")); //27.2C
			
			record = new Temperature { Value = 23, Date = DateTime.Parse("3/13/2001") };
			Assert.That(record.ToString(0), Is.EqualTo("010313 00 A  -50"));  //-5C
			
			record = new Temperature { Value = 41, Date = DateTime.Parse("3/13/2001") };
			Assert.That(record.ToString(0), Is.EqualTo("010313 00 A   50")); //5C
			
			record = new Temperature { Value = 122, Date = DateTime.Parse("3/13/2001") };
			Assert.That(record.ToString(0), Is.EqualTo("010313 00 A  500")); //50C
			
			record = new Temperature { Value = 42, Date = DateTime.Parse("3/13/2001") };
			Assert.That(record.ToString(0), Is.EqualTo("010313 00 A   56")); //5.6C
			
			record = new Temperature { Value = 32, Date = DateTime.Parse("3/13/2001") };
			Assert.That(record.ToString(0), Is.EqualTo("010313 00 A    0"));
			
			record = new Temperature { Value = 21, Date = DateTime.Parse("2/14/1997") };
			Assert.That(record.ToString(0), Is.EqualTo("970214 00 A  -61")); //-6.1C
			
			record = new Temperature { Value = 3, Date = DateTime.Parse("2/14/1997") };
			Assert.That(record.ToString(0), Is.EqualTo("970214 00 A -161")); //-16.11C
			
			record = new Temperature { Value = 31, Date = DateTime.Parse("2/14/1997") };
